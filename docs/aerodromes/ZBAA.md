...
| 运行模式 |   起飞跑道   |   落地跑道   | 备注                                                                                |
| -------- | :----------: | :----------: | ----------------------------------------------------------------------------------- |
| 北向运行 | 36L, 36R, 01 | 36L, 36R, 01 | 任意两条平行跑道可以实施相关平行仪表进近，36L 跑道、01 跑道可以实施独立平行仪表进近 |
| 南向运行 | 18R, 18L, 19 | 18R, 18L, 19 | 任意两条平行跑道可以实施相关平行仪表进近                                            |

#### 2.4.2 低能见度运行

> [!IMPORTANT]
> 适用条件：机场能见度 VIS 小于 800m, 或任一跑道视程 RVR 小于 550m，或云高低于 60m 时
>
> 北京首都机场低能见度运行仅适用于北向运行

| 跑道 | RVR550-450 | RVR450-400 | RVR400-300 | RVR300-175 | RVR175-150 | RVR150-90 |
| :--: | :--------: | :--------: | :--------: | :--------: | :--------: | :-------: |
| 36L  | 起飞、降落 |    起飞    |    关闭    |    关闭    |    关闭    |   关闭    |
| 36R  | 起飞、降落 | 起飞、降落 | 起飞、降落 | 起飞、降落 |    起飞    |   关闭    |
|  01  | 起飞、降落 | 起飞、降落 | 起飞、降落 |    起飞    |    起飞    |   起飞    |
...
01/19 跑道实施限制运行，01 跑道不允许航空器降落，19 跑道不允许航空器起飞
...
以机场基准点为中心半径15km的圆，高度600m（MSL）（含）以下的范围
...
![塔台管制范围](https://i.see.you/2026/03/12/6ePo/vatprc-2026-03-12-01KKG0A9SSR49F)
图 3.1 塔台管制范围划分图（橙色部分为 ZBAA_W_RMP 代管区域）
...
![机坪管制范围](https://i.see.you/2026/03/12/gk2C/vatprc-2026-03-12-01KKG093JGGN70)
...
4. 北京首都机场具有实施缩小航空器起飞着陆间隔的能力，在满足下列条件时可以实施缩小航空器起飞着陆间隔：
   - **同一跑道前行离场航空器与后随离场航空器使用不同离场程序**
...
5. 同一跑道航空器复飞后，在复飞航空器通过跑道头后 2 分钟后放飞。满足缩小航空器起飞着陆间隔时，在复飞航空器距起飞航空器 2400 米以上时，可以许可航空器起飞。
6. 同一跑道前行航空器执行起落航线的，在前行航空器一转弯后许可后随航空器起飞。若后随航空器同样执行起落航线，则在前行航空器加入三边后许可后随航空器起飞。
7. 需要雷达引导的航空器，塔台管制员在许可航空器进入跑道前必须向当前正在对终端管制区提供管制服务的管制席位通报雷达引导需求。

#### 4.1.2 进场航空器的处理

> [!IMPORTANT]
> 对于使用 36R/18L 跑道落地的航空器，**需指示脱离方向**。
...
|  管制席位  |              先决条件              |        移交位置         |  移交对象  |
| :--------: | :--------------------------------: | :---------------------: | :--------: |
|  ZBAA_TWR  |          36R 跑道向东脱离          |   给出滑行方向后移交    |  ZBAA_GND  |
|  ZBAA_TWR  |        沿 A9 穿越 36R 跑道         |   给出滑行方向后移交    |  ZBAA_GND  |
|  ZBAA_TWR  |          36R 跑道向西脱离          |   给出滑行方向后移交    | ZBAA_W_RMP |
|  ZBAA_TWR  |        沿 A8 穿越 36R 跑道         |   给出滑行方向后移交    | ZBAA_W_RMP |
|  ZBAA_TWR  |          18L 跑道向西脱离          |        Z3/M7 前         | ZBAA_W_RMP |
|  ZBAA_TWR  |          A1 穿越 18L 跑道          |        Z3/M7 前         | ZBAA_W_RMP |
|  ZBAA_TWR  |      18L 跑道沿 E0/E1/E2 脱离      |     脱离后立即移交      |  ZBAA_RMP  |
|  ZBAA_TWR  | 18L 跑道向东（除 E0/E1/E2 外）脱离 |   给出滑行方向后移交    |  ZBAA_GND  |
|  ZBAA_TWR  |  A0 穿越 18L 跑道后右转沿 G 滑行   |          E2 前          |  ZBAA_RMP  |
|  ZBAA_TWR  |    A0 穿越 18L 跑道（不向右转）    |          G 前           |  ZBAA_GND  |
| ZBAA_E_TWR |            01 跑道脱离             |   给出滑行方向后移交    |  ZBAA_GND  |
| ZBAA_E_TWR |        19 跑道沿 Q3/Q4 脱离        |          K 前           |  ZBAA_GND  |
| ZBAA_E_TWR |         19 跑道沿 Q2 脱离          |   给出滑行方向后移交    |  ZBAA_GND  |
| ZBAA_E_TWR |        19 跑道沿 Q0/Q1 脱离        |          K 前           |  ZBAA_RMP  |
| ZBAA_W_TWR |             沿 C 滑行              | 在预计向东转向的道口前① | ZBAA_W_RMP |

> [!NOTE]
>
> ①预计向东转向的道口包括：Z2、C1、C2、M、S5、C6、C7、C8

#### 4.1.7 与对进近管制区提供雷达管制服务的席位的移交优先级

| 优先级 |    席位     |  频率   |
| :----: | :---------: | :-----: |
|   1    | ZBAA_AA_APP | 119.000 |
|   2    |  ZBAA_APP   | 120.600 |
|   3    |  ZBAA_CTR   | 126.700 |
...
![北向滑行路线](https://i.see.you/2026/03/12/U5qx/vatprc-2026-03-12-01KKG01BAC37X6)
图 4.1 北向滑行图（红线表离场、蓝线表进场、绿线表进离场皆可、箭头代表运行方向）

##### 南向运行

![南向滑行路线](https://i.see.you/2026/03/12/1Noh/vatprc-2026-03-12-01KKG05B6WPB07)
...
| 运行方向 |        移交位置         |  移交对象  |
| :------: | :---------------------: | :--------: |
|   北向   |         K Q1 前         | ZBAA_E_TWR |
|   北向   |         G E2 前         |  ZBAA_TWR  |
|   北向   |         G A8 前         |  ZBAA_TWR  |
|   南向   |         K Q8 前         | ZBAA_E_TWR |
|   南向   | J（M 维修坪向南） U2 前 | ZBAA_E_TWR |
|   南向   |         G E7 前         |  ZBAA_TWR  |
|   南向   |  H（北绕滑向南） H4 前  |  ZBAA_TWR  |

##### 与机坪的移交

| 运行方向  |    移交位置    |  移交对象  |
| :-------: | :------------: | :--------: |
| 北向/南向 |    G H1 前     | ZBAA_W_RMP |
| 北向/南向 |    T1 Y2 前    |  ZBAA_RMP  |
| 北向/南向 |    T2 Y5 前    |  ZBAA_RMP  |
| 北向/南向 | T3/G4/G5 Y1 前 |  ZBAA_RMP  |
| 北向/南向 | T4/K4/K5 Y4 前 |  ZBAA_RMP  |
| 北向/南向 |    J2 Y3 前    |  ZBAA_RMP  |
| 北向/南向 |    J3 Y6 前    |  ZBAA_RMP  |
| 北向/南向 |   H/H7 Y8 前   |  ZBAA_RMP  |
| 北向/南向 |   J Y9/U2 前   |  ZBAA_RMP  |
|   南向    |    G G2 前     |  ZBAA_RMP  |
|   南向    |    K K2 前     |  ZBAA_RMP  |
...
#### 4.3.1 推出至带有运行方向限制的滑行道

部分机位推出时可能需要推出至带有运行方向限制的滑行道，滑行道方向限制见 4.2.1 滑行道的使用
相应机位推出位置与朝向应当参照以下规则：

- 优先指定不干扰带限制滑行道运行的推出位置
- 目标滑行道为 Z0/Z9/Z3/S6/Y7 的：
  - 优先指定和目标滑行道运行方向限制相同的推出方向
  - 需要指定与目标滑行道运行方向限制不同的推出方向时，应指挥航空器就近脱离推出时占用的滑行道
- 目标滑行道为 Z18/J2/J3/Y3/Y6/G3/G4/K4/K5 的：
  - 严格按照滑行道运行方向限制指定朝向
...
| ZBAA_W_RMP |   北向    |        沿 Z3 向南滑行，预计进入 M7 以南的 F 滑行道的         | 在预计向东转向的道口前① |  ZBAA_TWR  |
| ZBAA_W_RMP |   北向    | 沿 M7/F7/F4/Z0/F1/F0 向东滑行，预计进入 M7 以南的 F 滑行道的 |          Z3 前          |  ZBAA_TWR  |
...
> [!NOTE]
>
> ①预计向东转向的道口包括：M7、F7、F4、F3、F2、F1、F0
...
1. DCL 登录名 ZBAA
2. DCL 可由北京首都机场除机坪席位以外的任一席位操作，默认由 ZBAA_DEL 操作
...
|  MUGLO  |              日韩（除 RKSI、RKSS 外）、北美              | 8900（不含）以上米制单数 |                              主用离场点                               |
| `LULTA` |                                                          |                          | 原则上不使用 必须由区域管制席位提出使用，进近管制席位同意后才可以使用 |
|  DOTRA  |                           ZYTX                           |        8100/8900         |                              主用离场点                               |
|  DOTRA  |                           ZYCC                           |        9500/8900         |                              主用离场点                               |
|  DOTRA  |              东北、西伯利亚东部、北美、朝鲜              |         米制单数         |                              主用离场点                               |
|  IDKEX  | ZLDH、内蒙古东部及北部、蒙古、欧洲、新疆、西伯利亚中西部 |         米制双数         |                              主用离场点                               |
|  BOTPU  |                           ZBHH                           |        6000/6600         |                              主用离场点                               |
|  BOTPU  |                        ZBDS、ZBOW                        |        7200/7800         |                              主用离场点                               |
|  BOTPU  |                           ZBDT                           |           4200           |                              主用离场点                               |
|  BOTPU  |            西北、青海、西藏、中亚、西亚、欧洲            |         米制双数         |                              主用离场点                               |
|  RUSDO  |                           ZLXY                           |           8400           |          应沿 RUSDO W45 SUMSO B215 飞行，不得加入 W276 航路           |
|  RUSDO  |                           ZBYN                           |      6600/7200/7800      |          应沿 RUSDO W45 SUMSO B215 飞行，不得加入 W276 航路           |
|  RUSDO  |                           西南                           |         米制双数         |          应沿 RUSDO W45 ATMIL W276 飞行，不得加入 B215 航路           |
|  RUSDO  |           华南、中原、西南、东南亚、南亚、ZSCN           |         米制双数         |                              主用离场点                               |
| `OMDEK` |                           ZBSJ                           |           4200           |                             非主用离场点                              |
| `PEGSO` |                                                          |                          |                             非主用离场点                              |

> [!IMPORTANT]
> 由北京首都机场出发沿京广大通道南下的航空器，不得由 ELKUR 方向离场后沿 W40 航路南下后沿 W562/W4 航路向西加入 W37/A461 航路，应由 RUSDO 方向离场。
> 由北京首都机场出发沿京蓉大通道南下的航空器，凡是经过 LOVRA 前往西安以远的目的地的，应使用 W276 航路，不应当使用 B215/W287 航路；前往西安、太原的应使用 B215/W287 航路，不应当使用 W276 航路。
...
| 机型   | 停机位                                                                                                                                    |
| :----- | :---------------------------------------------------------------------------------------------------------------------------------------- |
| A380   | 507-509、701、702、931、932、938、939、951、955、M01、M02                                                                                 |
...
![A380、B747-8、AN124 可运行滑行道图](https://i.see.you/2026/03/12/g0Ho/vatprc-2026-03-12-01KKFV5GM9VF90)
图 5.1 A380、B747-8、AN124 可运行滑行道图（橙色线条为可运行滑行道）
...
机场能见度 VIS 小于 800m, 或任一跑道视程 RVR 小于 550m，或云高低于 60m 时

#### 5.2.1 可运行区域

36L/18R 跑道及其联络道除外的所有区域

#### 5.2.2 低能见度滑行路线（仅北向）：

![低能见运行](https://i.see.you/2026/03/12/kTo8/vatprc-2026-03-12-01KKG073W6P3M6)
图 5.2 北向低能见度滑行图（红线表离场、蓝线表进场、绿线表进离场皆可、箭头代表运行方向）

##### 跑道联络道使用限制

- 36L/18R 跑道的所有跑道联络道不得使用
- A0、A1 联络道关闭，不得使用
- 36R 跑道 RVR 300-175 时，应当尽可能避免使用 M7 以南的 F 滑行道及 T5 以南的 G 滑行道
- 01 跑道 RVR 200-90 时，应当尽可能避免使用 K7 以南的 K 滑行道

> [!NOTE]
> 出现航空器误入跑道联络道时，应立刻停止对应跑道的一切起落活动，立即清空跑道并指挥误入航空器离开联络道，然后重新恢复正常低能见度运行

#### 5.2.3 塔台席位与其他场面席位的移交

|  管制席位  |                 先决条件                 |    移交位置     |  移交对象  |
| :--------: | :--------------------------------------: | :-------------: | :--------: |
|  ZBAA_TWR  | 向东脱离 36R 跑道或沿 A9 穿越 36R 跑道后 |      G 前       |  ZBAA_GND  |
|  ZBAA_TWR  | 向西脱离 36R 跑道或沿 A8 穿越 36R 跑道后 |      F 前       | ZBAA_W_RMP |
| ZBAA_E_TWR |              脱离 01 跑道后              |      K 前       |  ZBAA_GND  |
| ZBAA_W_TWR |       沿 C 向北滑行去往 W5/W6 机坪       |    C7/C8 前     | ZBAA_W_RMP |
| ZBAA_W_TWR |    沿 C 向南滑行去往 36R/01 跑道离场     | S5/M/Z2(HP6) 前 | ZBAA_W_RMP |

#### 5.2.4 地面席位与其他场面席位的移交
...
|    T1 Y2 前    |  ZBAA_RMP  |
|    T2 Y5 前    |  ZBAA_RMP  |
| T3/G4/G5 Y1 前 |  ZBAA_RMP  |
| T4/K4/K5 Y4 前 |  ZBAA_RMP  |
|    J2 Y3 前    |  ZBAA_RMP  |
|    J3 Y6 前    |  ZBAA_RMP  |
|   H/H7 Y8 前   |  ZBAA_RMP  |
|   J Y9/U2 前   |  ZBAA_RMP  |
...
> [!CAUTION]
> 航空器使用 A8 联络道穿越 36R 跑道，需取得 ZBAA_TWR 批准
> 航空器未联系塔台，不得进入联络道，不得跨越 CATⅡ/Ⅲ 等待点

#### 5.2.5 机坪席位与其他场面席位的移交

|  管制席位  |                    先决条件                    | 移交位置 |  移交对象  |
| :--------: | :--------------------------------------------: | :------: | :--------: |
|  ZBAA_RMP  |            沿 T1/T3/Y4/Y9 向东滑行             |   J 前   |  ZBAA_GND  |
|  ZBAA_RMP  |            沿 T2/T4/Y1/Y8 向西滑行             |   H 前   |  ZBAA_GND  |
|  ZBAA_RMP  |                 沿 Y3 向西滑行                 |  J1 前   |  ZBAA_GND  |
|  ZBAA_RMP  |                 沿 Y6 向东滑行                 |  J4 前   |  ZBAA_GND  |
|  ZBAA_RMP  |                 沿 J 向南滑行                  |  U2 前   |  ZBAA_GND  |
|  ZBAA_RMP  |              沿 G0/G1/G2 向西滑行              |   G 前   |  ZBAA_TWR  |
|  ZBAA_RMP  |               沿 K1/K2 向东滑行                |   K 前   | ZBAA_E_TWR |
| ZBAA_W_RMP |       预计进入 C 滑行道（含）以西区域的        |   C 前   | ZBAA_W_TWR |
| ZBAA_W_RMP |            沿 Route2 （Y7）向南滑行            |  H2 前   |  ZBAA_GND  |
| ZBAA_W_RMP | 不在 F 滑行道上，预计进入 M7 以南的 F 滑行道的 |   F 前   |  ZBAA_TWR  |
| ZBAA_W_RMP |                 沿 F 向南滑行                  |  M7 前   |  ZBAA_TWR  |

> [!TIP]
> ZBAA_W_RMP 可以在确认 C 上滑行的航空器无冲突后，直接指挥 W211-W213 机位停靠的航空器沿 C Z2滑行，或指挥 W211-W213 机位停靠的航空器推出以避免进入 C
...
|                       |                                 |                           |
| :-------------------- | :------------------------------ | :------------------------ |
| 北部湾航空（CBG）     | 中国国际航空（CCA）（部分航线） | 湖南航空（OTC）           |
| 桂林航空（CGH）       | 柬埔寨天空吴哥航空（SWM）       | 济州航空（JJA）           |
| 伊朗马汉航空（IRM）   | 阿塞拜疆航空（AHY）             | 俄罗斯北方之星航空（TYA） |
| 四川航空（CSC）       | 宿务太平洋航空（CEB）           | 达美航空（DAL）           |
| 海南航空（CHH）       | 中国东方航空（CES）             | 大韩航空（KAL）           |
| 法国航空（AFR）       | 乌兹别克斯坦航空（UZB）         | 荷兰皇家航空（KLM）       |
| 阿斯塔纳航空（KZR）   | 高丽航空（KOR）                 | 巴基斯坦航空（PIA）       |
| 印度尼西亚鹰航（GIA） | 斯里兰卡航空（ALK）             | 土库曼斯坦航空（TUA）     |
| 越南航空（HVN）       | 阿尔及利亚航空（DAH）           | 香港航空（CRK）           |
| 大新华航空（GDC）     | 祥鹏航空（LKE）                 | 福州航空（FZA）           |
| 厦门航空（CXA）       | 中国南方航空（CSN）             |                           |
...
|                     |                       |                       |
| :------------------ | :-------------------- | :-------------------- |
| 曼谷航空（BKP）     | 吉祥航空（DKH）       | 意大利航空（ITY）     |
| 南非航空（SAA）     | 国泰港龙航空（HDA）   | 新西伯利亚航空（SBI） |
| 华夏航空（HXA）     | 新西兰航空（ANZ）     | 柬埔寨吴哥航空（KHV） |
| 东海航空(EPA)       | 昆明航空(KNA)         | 浙江长龙航空（CDC）   |
| 青岛航空（QDA）     | 西藏航空（TBA）       | 波兰航空（LOT）       |
| 中国国际航空（CCA） | 瑞士航空（SWR）       | 大连航空（CCD）       |
| 山东航空（CDG）     | 奥地利航空（AUA）     | 北欧航空（SAS）       |
| 汉莎航空（DLH）     | 韩亚航空（AAR）       | 加拿大航空（ACA）     |
| 美国联合航空（UAL） | 全日本航空（ANA）     | 土耳其航空（THY）     |
| 埃及航空（MSA）     | 泰国国际航空（THA）   | 新加坡航空（SIA）     |
| 芬兰航空（FIN）     | 国泰航空（CPA）       | 日本航空（JAL）       |
| 阿联酋航空（UAE）   | 以色列航空（ELY）     | 澳门航空（AMU）       |
| 中华航空（CAL）     | 长荣航空（EVA）       | 深圳航空（CSZ）       |
| 蒙古航空（MGL）     | 埃塞俄比亚航空（ETH） | 菲律宾航空（PAL）     |
