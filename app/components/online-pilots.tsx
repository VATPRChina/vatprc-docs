...
const Pilot: React.FC<{
  ...
  return (
    <div className="flex flex-col rounded-md border px-6 py-4 hover:bg-secondary">
...
export const OnlinePilots: React.FC<{ className?: string }> = ({ className }) => {
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="space-y-2">
      <div className={cn(className, "flex flex-wrap justify-center gap-x-2 gap-y-4")}>{pilots?.slice(0, 8)}</div>
      <CollapsibleContent>
        <div className={cn(className, "flex flex-wrap justify-center gap-x-2 gap-y-4")}>{pilots?.slice(8)}</div>
      </CollapsibleContent>
