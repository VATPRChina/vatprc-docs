...
import { Alert, Button, Card, Group, Select, TextInput, Textarea, UnstyledButton } from "@mantine/core";
...
export const Route = createFileRoute("/sheets")({
  validateSearch: (search: Record<string, unknown>): SheetsSearch => ({
...
function RouteComponent() {
  ...
  const selectedId = search.id ?? sheets?.[0]?.id;
  const { data: sheet, isLoading: isSheetLoading } = $api.useQuery(
    "get",
    "/api/sheets/{sheetId}",
    { params: { path: { sheetId: selectedId ?? "" } } },
    { enabled: !!selectedId },
  );
  ...
  useEffect(() => {
    if (search.id || !sheets?.[0]?.id) return;
    void navigate({
      search: { id: sheets[0].id },
      replace: true,
    });
  }, [navigate, search.id, sheets]);
              ...
              {sheets?.map((currentSheet) => (
                <UnstyledButton
                  ...
                  className={`rounded border px-3 py-2 text-left ${
                    currentSheet.id === selectedId ? "border-blue-500 bg-blue-50" : "border-default"
                  }`}
                  onClick={() => void navigate({ search: { id: currentSheet.id }, replace: true })}
                >
                  <div className="font-medium">{currentSheet.name}</div>
                          ...
                          <Textarea
                            ...
                            onChange={(event) =>
                              setFields((prev) =>
                                prev.map((current, currentIndex) =>
                                  currentIndex === index
                                    ? { ...current, description_zh: event.currentTarget.value }
                                    : current,
                                ),
                              )
                            }
                          ...
                          <Textarea
                            ...
                            onChange={(event) =>
                              setFields((prev) =>
                                prev.map((current, currentIndex) =>
                                  currentIndex === index
                                    ? { ...current, description_en: event.currentTarget.value }
                                    : current,
                                ),
                              )
                            }
