...
const PermissionTag: React.FC<{
  permission: ControllerPositionPermission;
  positionName: string;
  expiration?: Date | null;
}> = ({ permission, positionName, expiration }) => {
      ...
      >
        ...
        {expiration && (
          <span className="text-xs">
            {t("controller-list.until")} {format(expiration, "yyyy-MM-dd")}
          </span>
        )}
      </span>
    )
  ...
  return (
    <div className="mt-4 flex flex-col gap-4">
      <div className="flex items-center gap-2">
        ...
        <label>{t("controller-list.show-absence")}</label>
      </div>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {controllers
          ...
          ?.map((ctr) => (
            <div
              key={ctr.id}
              className="flex flex-col gap-4 rounded-md border px-6 py-4 hover:bg-gray-50"
            >
              <div className="flex items-center gap-2">
